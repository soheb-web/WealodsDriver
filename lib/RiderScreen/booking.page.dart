...
class _BookingPageState extends State<BookingPage> {
  ...
  Future<void> _fetchDeliveryHistory() async {
    try {
      ...
      if (response.code == 0 &&
          response.data?.deliveries != null &&
          response.data!.deliveries!.isNotEmpty) {
        setState(() {
  ...
  List<String> _extractNames(dynamic dropoff) {
    ...
    if (dropoff is Dropoff) {
      list.add(
        dropoff.name?.isNotEmpty == true ? dropoff.name! : "Drop Location",
      );
    } else if (dropoff is List) {
      for (var d in dropoff.take(3)) {
        if (d is Dropoff) {
  ...
  String _getDropoffName(dynamic drop, int index) {
    ...
    if (drop is Pickup) {
      return drop.name?.isNotEmpty == true
          ? drop.name!
