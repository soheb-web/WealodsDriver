...
class _HomePageState extends State<HomePage>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed && isDriverOnline) {
      Future.delayed(const Duration(milliseconds: 800), () {
        if (mounted) _ensureSocketConnected();
      });
    }
    ...
    try {
      ...
      if (response.error == false && response.data != null && mounted) {
        setState(() {
          ...
          statusFront = response.data!.driverDocuments?.identityFront?.status ?? '';
          statusBack = response.data!.driverDocuments?.identityBack?.status ?? '';
  ...
  void _registerAndSendLocation() async {
    if (!socket!.connected || driverId == null) return;
  ...
  // ==================== MULTIPLE REQUESTS WITH 10 SEC TIMER ====================
          ...
          child: AlertDialog(
            ...
            title: Row(
              children: [
                ...
                SizedBox(width: 12.w),
                Text("New Requests (${_activeRequests.length})",
            ...
            content: SizedBox(
              ...
              child: _activeRequests.isEmpty
                ...
                itemBuilder: (context, i) => _buildRequestCard(_activeRequests[i]),
              ),
            ),
            actions: [
              TextButton(
          ...
          children: [
            Row(
              children: [
                ...
                SizedBox(width: 8.w),
                Expanded(
                  child: Text("Pickup: ${req.pickupName}",
                      style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14.sp)),
                ),
              ],
            ),
            SizedBox(height: 8.h),
            ...req.dropOffLocations.map((name) => Padding(
              padding: EdgeInsets.only(left: 26.w, top: 4.h),
              child: Row(
                children: [
                  Icon(Icons.subdirectory_arrow_right, color: Colors.green, size: 16.sp),
                  SizedBox(width: 6.w),
                  Expanded(child: Text(name, style: TextStyle(fontSize: 13.sp))),
                ],
              ),
            )),
  ...
  // void _removeRequest(String id, {bool autoExpired = false, bool rejected = false}) {
  //   if (!mounted) return;
  ...
  void _removeRequest(String id, {bool autoExpired = false, bool rejected = false}) {
    ...
    NotificationService.instance.stopBuzzer(); // 👈 YE LINE SABSE UPAR DAAL DO
  ...
  void _acceptAndRemove(String id) {
    _acceptDelivery(id);
    _removeRequest(id);
    NotificationService.instance.stopBuzzer();  // Yeh add kar do
  ...
  void _rejectAllRequests() {
    ...
    NotificationService.instance.stopBuzzer();  // Yeh hona chahiye
...
// 🔥 BACK AANE KE BAAD — REFRESH EVERYTHING
        if (!mounted) return;

// 1️⃣ API refresh
        await getDriverProfile();

// 2️⃣ Socket ensure / reconnect
        _ensureSocketConnected();

// 3️⃣ Fresh booking request fire (location ke sath)
        final pos = await _getCurrentLocation();
  ...
  void didUpdateWidget(covariant HomePage oldWidget) {
    ...
    // 🔥 Jab forceSocketRefresh false → true ho
    if (widget.forceSocketRefresh && !oldWidget.forceSocketRefresh) {
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                InkWell(
                  ...
                  child: Container(
                    width: 35.w,
                    height: 35.h,
                    decoration: BoxDecoration(shape: BoxShape.circle, color: Color(0xFFA8DADC)),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  Row(
                    children: [
                      Icon(isDriverOnline ? Icons.circle : Icons.circle_outlined,
                          color: isDriverOnline ? Colors.green : Colors.red, size: 16.sp),
                      SizedBox(width: 6.w),
                      Text(isDriverOnline ? "ONLINE" : "OFFLINE",
                          style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.bold, color: isDriverOnline ? Colors.green : Colors.red)),
                    ],
                  ),
                  Switch(
            ...
            if (statusFront == "pending" && statusBack == "pending")
              GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => IdentityCardPage()))
            ...
            if (vehicleList.isEmpty)
              GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => VihicalPage()))
                    .then((_) => getDriverProfile()),
    ...
    physics: const AlwaysScrollableScrollPhysics(),
                child: Column(
                  ...
                  children: [
                    ...
                    availableRequests.isEmpty ? _buildEmptyState() : _buildRequestList(),
                  ],
                ),
              ),
            ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          Text("Driver: ${isSocketConnected ? 'Online' : 'Offline'}",
              style: TextStyle(color: isSocketConnected ? Colors.green : Colors.red, fontSize: 12.sp)),
        ],
      ),
    );
  }
      ...
      itemBuilder: (context, index) {
        ...
        if (dropoffData != null) {
          if (dropoffData is List) {
            dropoffNames = dropoffData.take(3).map((d) => (d as Map)['name']?.toString() ?? 'Unknown').toList();
          } else if (dropoffData is Map) {
            dropoffNames = [(dropoffData['name']?.toString() ?? 'Unknown')];
          }
        }
        final dropoffText = dropoffNames.isEmpty ? "No dropoff" : dropoffNames.join(" → ");
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Expanded(child: Text("Pickup: $pickup", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14.sp))),
                    Text("₹$price", style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600, fontSize: 16.sp)),
                  ],
                ),
                SizedBox(height: 8.h),
                Text("Dropoff: $dropoffText", style: TextStyle(fontSize: 13.sp)),
                SizedBox(height: 6.h),
                ...
                Row(
                  ...
                  children: [
                    ...
                    OutlinedButton(
                        onPressed: () =>
                            _rejectDelivery(req['_id'] ?? req['deliveryId']), child: const Text("Reject")),
                  ],
                ),
              ],
            ),
  ...
  void _deliveryAcceptDelivery(String deliveryId) {
    if (socket != null && socket!.connected) {
      socket!.emitWithAck('delivery:accept_request', {'deliveryId': deliveryId}, ack: (_) {});
      Fluttertoast.showToast(msg: "Delivery Accepted!");
    }
  }
...
class HomePage extends StatefulWidget {
  ...
  const HomePage(this.selectIndex, {super.key, this.forceSocketRefresh = false});
...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // Profile load karo — connect logic getDriverProfile ke andar handle hoga
    getDriverProfile();
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      if (mounted && isSocketConnected && driverId != null && driverId!.isNotEmpty) {
        Future.delayed(const Duration(milliseconds: 800), _ensureSocketConnected);
      }
    }
    if (state == AppLifecycleState.paused) {
      _locationTimer?.cancel();
  ...
  void _connectSocket() {
    ...
    print("connectSocket called | driverId = $driverId");
    ...
    socket!.onConnect((_) {
      print("Socket successfully connected → should show ONLINE now");
      if (mounted) {
        setState(() {
    ...
    socket!.onConnectError((err) {
      ...
      if (mounted) {
        ...
        Fluttertoast.showToast(msg: "Connection failed. Please try again.");
      }
    });
  ...
  Future<void> getDriverProfile() async {
    try {
      ...
      if (response.error == false && response.data != null && mounted) {
        ...
        // Force refresh case mein yahan se connect attempt karo
  */
/*      if (widget.forceSocketRefresh && driverId != null && driverId!.isNotEmpty) {
          print("forceSocketRefresh = true → auto connecting socket");
          Future.delayed(const Duration(milliseconds: 300), () {
            if (mounted) {
              _disconnectSocket(); // safety
              _connectSocket();
            }
          });
        }*//*
        ...
        // getDriverProfile ke andar force block mein
        if (widget.forceSocketRefresh && driverId != null && driverId!.isNotEmpty) {
          print("forceSocketRefresh = true → auto connecting socket");
          Future.delayed(const Duration(milliseconds: 300), () {
            if (mounted) {
              setState(() {
                isSocketConnected = true;  // ← optimistic: UI ko pehle ONLINE dikha do
                _isConnecting = false;
              });
              _disconnectSocket();
              _connectSocket();  // real attempt chalao
            }
          });
        }
      }
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _isConnecting
                    ...
                    : (isSocketConnected ? Colors.green.withOpacity(0.15) : Colors.red.withOpacity(0.15)),
                borderRadius: BorderRadius.circular(20.r),
                border: Border.all(
                  color: _isConnecting
                      ? Colors.orange
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (_isConnecting)
                    ...
                    Switch(
                      value: isSocketConnected,
                      activeColor: Colors.green,
                      inactiveThumbColor: Colors.red,
                      inactiveTrackColor: Colors.red.withOpacity(0.5),
                      onChanged: (driverId == null || driverId!.isEmpty)
                          ? null
                          : (bool value) async {
                        if (value) {
                          await getDriverProfile();
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          const Spacer(),
                          Text("View all",
                              style: GoogleFonts.inter(fontSize: 13.sp, color: Color(0xFF006970))),
                        ],
                      ),
                      availableRequests.isEmpty ? _buildEmptyState() : _buildRequestList(),
                    ],
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          Text(
            "Driver: ${isSocketConnected ? 'Connected' : 'Disconnected'}",
            style: TextStyle(color: isSocketConnected ? Colors.green : Colors.red, fontSize: 12.sp),
          ),
        ],
      ),
  ...
  Future<void> _rejectDelivery(String deliveryId) async {
    try {
      final body = RejectDeliveryBodyModel(deliveryId: deliveryId, lat: latitude.toString(), lon: longitude.toString());
      final service = APIStateNetwork(await callDio());
      await service.rejectDelivery(body);
      _refreshHomeCompletely();
    } catch (e) {}
...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  void _connectSocket() {
    ...
    print("connectSocket called | driverId = $driverId | forceRefresh = ${widget.forceSocketRefresh}");
              ...
              builder: (context, connected, child) {
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color: _isConnecting
                        ? Colors.orange.withOpacity(0.15)
                        : (connected ? Colors.green.withOpacity(0.15) : Colors.red.withOpacity(0.15)),
                    borderRadius: BorderRadius.circular(20.r),
                    border: Border.all(
                      color: _isConnecting
                          ? Colors.orange
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      availableRequests.isEmpty
                        ...
                        valueListenable: _connectionNotifier,
                        builder: (context, connected, child) {
                          return _buildEmptyState(connected: connected);
                        },
                      )
                          : _buildRequestList(),
                    ],
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(color: connected ? Colors.green : Colors.red, fontSize: 12.sp),
          ),
        ],
      ),
    );
